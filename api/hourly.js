export default async function handler(req, res) {
  ...
  const url = `https://api.weather.com/v2/pws/observations/hourly/1day?stationId=${stationId}&format=json&units=m&apiKey=${apiKey}&numericPrecision=decimal`;

  try {
    const response = await fetch(url);
    const bodyText = await response.text();
    console.log('[hourly]', stationId, 'HTTP', response.status, bodyText.slice(0, 200));
